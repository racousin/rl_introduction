...
class Agent:
    def __init__(self, env, gamma=0.99, epsilon=0.01):
        self.env = env
        self.policy = (
...
def policy_improvement(env, V, gamma=1):
    policy = (
        np.zeros([env.observation_space.n, env.action_space.n]) / env.action_space.n
    )
    for s in range(env.observation_space.n):
        q = q_from_v(env, V, s, gamma)
        best_a = np.argwhere(q == np.max(q)).flatten()
        policy[s] = np.sum(
            [np.eye(env.action_space.n)[i] for i in best_a], axis=0
        ) / len(best_a)
